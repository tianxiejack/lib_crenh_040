...
static cudaError_t cudaMemcpyAsync_share(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind, int chn, int flag, cudaStream_t stream)
{
	cudaError_t ret = cudaSuccess;
	OSA_assert(chn >= 0 && chn < SHAREARRAY_CNT);
	OSA_assert(dst != NULL);
	...
	if(kind == cudaMemcpyDeviceToHost ){
		ret = cudaMemcpyAsync(dst, src, count, kind, stream);
	}else if(kind == cudaMemcpyDeviceToHost || shareArray[chn].iWrite == 0 || flag != 0){
		shareArray[chn].iWrite ++;
		ret = cudaMemcpyAsync(dst, src, count, kind, stream);
	}
...
static cudaError_t cudaResource_RegisterBuffer(int index, unsigned int buffer, size_t size)
{
	cudaError_t ret = cudaSuccess;
	int i;
