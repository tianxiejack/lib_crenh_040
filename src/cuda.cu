	...
	int width, int height, cudaStream_t stream)
{
	...
	//dim3 thread(128, 128);
	kernel_dei<<<block, thread, 0, stream>>>(dst, width, height);
...
__global__ void kernel_uyvy2gray(
	unsigned char *dst, const unsigned char *src,
	int width, int height)
{
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	...
	dst[y*width + x] = src[y*width*2 + x*2 + 1];
}
	...
	int width, int height, cudaStream_t stream)
{
	...
	kernel_uyvy2gray<<<block, thread, 0, stream>>>(dst, src, width, height);
...
#if 0
__global__ void kernel_rgbResize(
		const unsigned char *src, unsigned char *dst,
			int srcw, int srch, int dstw, int dsth, float ratiox, float ratioy)
{
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	int y = blockDim.y * blockIdx.y + threadIdx.y;
	...
	fx = x*ratiox;		fy = y*ratioy;
	ix = (int)fx;			iy = (int)fy;
	a_01 = fx - ix;		a_00 = 1.f - a_01;
	b_01 = fy - iy;		b_00 = 1.f - b_01;
	if( (ix+1) >= srcw || (iy+1) >= srch)
		return;
	unsigned char *psrc, *pdst;
	for(i=0; i<3; i++){
		psrc = src+iy*srcw*3+ix*3+i;
		pdst = dst+y*dstw*3+x*3+i;
		C00 = *(psrc);C01 = *(psrc+1);C10 = *(psrc+srcw*3);C11 = *(psrc+srcw*3+1);
		gray = (int)((C00*a_00 + C01*a_01)*b_00 + (C10*a_00 + C11*a_01)*b_01);
		*pdst = clip(0, 255, gray);
	}
}
...
{
	dim3 block((dstw/2+31)/32,(dsth+31)/32);
	dim3 thread(32, 32);
	float	ratiox = dstw*1.0/srcw;
	float	ratioy = dsth*1.0/srch;
	kernel_rgbResize<<<block, thread>>>(src, dst, srcw, srch, dstw, dsth, ratiox, ratioy);
